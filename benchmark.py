...
DEFAULT_FILE = Path(__file__).parent.joinpath("lib/test/spec.txt")
...
def benchmark(package_name):
    def decorator(func):
        def inner(text, num_parses):
            try:
                if package_name == "markdown-it-py":
                    from lib.main import MarkdownIt
...
@benchmark("markdown-it")
def run_markdown_it_py(package, text):
    md = package("commonmark")
